...
export default function PerformanceAccessibilitySection({ section, ctx }: Props) {
  const { colors, onImageClick, getImageStyle, onImageLoad } = ctx;
  const blocks = section.performanceAccessibilityBlocks!;
        ...
        >
          ...
          <ul className="space-y-2 list-none pl-0">
            {block.body.map((paragraph, j) => (
              <li
                key={j}
                className="font-light leading-relaxed opacity-90 flex gap-2"
